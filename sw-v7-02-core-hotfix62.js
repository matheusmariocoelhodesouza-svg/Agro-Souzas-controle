const CACHE='comando360-v7-02-hotfix62';
...
const CORE=[
  ...
  './c360-quality-hotfix.js','./c360-quality-core.js','./c360-runtime-compatibility.js','./c360-final-stabilization.js','./c360-data-integrity.js','./c360-team-chat.js','./c360-farm-cache-hotfix.js',
  ...
  './c360-system-health.css','./c360-ui-polish.css','./c360-premium-ui.css','./c360-product-ui.css','./c360-release-core.css','./c360-final-stabilization.css',
  ...
  './c360-mobile-rescue.css','./c360-signature-ui.css','./c360-signature-icons.css','./c360-professional-pass.css','./c360-signature-ui.js','./c360-professional-pass.js'
];
...
async function cacheOne(cache,url,timeout=3200){
  try{
    ...
    if(response&&response.ok){await cache.put(url,response.clone());return true}
  }catch(_){}
  return false;
}
...
async function getMeta(){
  try{const c=await caches.open(META_CACHE);const r=await c.match(META_KEY);return r?await r.json():{}}catch(_){return{}}
}
async function setMeta(patch){
  const current=await getMeta();
  const next={...current,...patch,updated_at:new Date().toISOString()};
  const c=await caches.open(META_CACHE);
  await c.put(META_KEY,new Response(JSON.stringify(next),{headers:{'Content-Type':'application/json','Cache-Control':'no-store'}}));
  return next;
}
async function copyCache(sourceName,targetName){
  ...
  await mapLimit(keys,CONCURRENCY,async req=>{const res=await source.match(req);if(res){await target.put(req,res.clone());copied++}});
  return copied>0;
}
async function stableMatch(request,shell=false){
  const cache=await caches.open(STABLE_CACHE);
  if(shell)return(await cache.match('./index.html'))||(await cache.match('./'))||null;
  const url=new URL(typeof request==='string'?request:request.url,self.location.origin);
  let hit=await cache.match(request);if(hit)return hit;
  if(url.origin===self.location.origin){
    hit=await cache.match(url.origin+url.pathname);if(hit)return hit;
    const scope=new URL(self.registration.scope);
    if(url.pathname.startsWith(scope.pathname)){
      const rel='./'+url.pathname.slice(scope.pathname.length).replace(/^\/+/, '');
      hit=await cache.match(rel);if(hit)return hit;
    }
  }
  return null;
}
...
async function prepareCore(){
  ...
  let refreshed=0,shellRefreshed=false;
  await Promise.all(SHELL.map(async url=>{if(await cacheOne(cache,url,4500)){refreshed++;shellRefreshed=true}}));
  let hasShell=!!((await cache.match('./index.html'))||(await cache.match('./')));
  if(!hasShell)throw new Error('APP_SHELL_NOT_READY');
  const rest=CORE.filter(x=>!SHELL.includes(x));
  await mapLimit(rest,CONCURRENCY,async url=>{if(await cacheOne(cache,url,3200))refreshed++});
  hasShell=!!((await cache.match('./index.html'))||(await cache.match('./')));
  return{ok:true,hasShell,refreshed,shellRefreshed};
}
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    const previous=keysBefore.filter(k=>/^comando360-v7-02-hotfix\d+$/.test(k)&&k!==CACHE).sort((a,b)=>versionNumber(b)-versionNumber(a))[0]||null;
    if(previous){try{await copyCache(previous,STABLE_CACHE)}catch(_){}}
    await prepareCore();
    const current=await caches.open(CACHE);const ready=(await current.match('./index.html'))||(await current.match('./'));
    if(ready){
      ...
      await Promise.all(keys.filter(k=>/^comando360-v7-02-hotfix\d+$/.test(k)&&k!==CACHE).map(k=>caches.delete(k)));
    }
...
self.addEventListener('message',event=>{
  ...
  const reply=data=>{try{event.ports&&event.ports[0]&&event.ports[0].postMessage(data)}catch(_){}};
  event.waitUntil((async()=>{
    try{
      ...
      if(msg.type==='C360_HEAL_CORE'){
        const result=await prepareCore();const healed=!!result.shellRefreshed&&result.refreshed>=3;
        await setMeta({last_heal_at:new Date().toISOString(),last_heal_reason:String(msg.reason||'').slice(0,240),last_heal_ok:healed});
        reply({ok:healed,...result});return;
      }
      if(msg.type==='C360_ROLLBACK_TO_STABLE'){
        const ready=await stableReady();if(!ready){reply({ok:false,reason:'stable-cache-unavailable'});return}
        const meta=await setMeta({rollback:true,rollback_at:new Date().toISOString(),rollback_reason:String(msg.reason||'').slice(0,240)});
        reply({ok:true,rollback:true,stable_from:meta.stable_from||null});return;
      }
      if(msg.type==='C360_MARK_HEALTHY'){
        const meta=await getMeta();if(!meta.rollback)await setMeta({last_healthy_at:new Date().toISOString(),healthy_build:String(msg.build||'').slice(0,80),recovery_version:String(msg.recovery_version||'').slice(0,80)});
        reply({ok:true,rollback:!!meta.rollback,current:CACHE,stable:STABLE_CACHE,stable_ready:await stableReady()});return;
      }
...
self.addEventListener('fetch',event=>{
  ...
  const url=new URL(req.url);if(url.hostname.endsWith('supabase.co'))return;
  if(req.mode==='navigate'){
    event.respondWith((async()=>{
      const shell=isAppShellNavigation(url);const meta=await getMeta();
      if(meta.rollback){const stable=await stableMatch(req,shell);if(stable)return stable}
      try{
        const fresh=await fetchWithTimeout(req,4000);
        if(fresh&&fresh.ok){const cache=await caches.open(CACHE);if(shell){await cache.put('./index.html',fresh.clone());await cache.put('./',fresh.clone())}else await cache.put(req,fresh.clone());return fresh}
      }catch(_){}
      const cache=await caches.open(CACHE);
      if(shell){const cached=(await cache.match('./index.html'))||(await cache.match('./'));if(cached)return cached}else{const cached=await cache.match(req);if(cached)return cached}
      return offlinePage();
    })());return;
  }
