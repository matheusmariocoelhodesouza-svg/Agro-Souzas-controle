(()=>{
'use strict';
const BOOT_VERSION='2026.09.12-b1';
const modules=[
...
async function boot(){
 ...
 window.__c360Bootstrap={version:BOOT_VERSION,modules:[...modules],errors,ready:errors.length===0};
 if(errors.length)console.error('Comando 360: módulos não carregados:',errors.join(' | '));
 document.dispatchEvent(new CustomEvent('c360:bootstrap-ready',{detail:window.__c360Bootstrap}));
}

boot().catch(e=>{
 window.__c360Bootstrap={version:BOOT_VERSION,modules:[...modules],errors:[e?.message||String(e)],ready:false};
